...
TOKEN = "5279623052:AAFUoXDxmwN1Y8mCXGLH7iyhJrQ0wK6R7IY"
...
def echo(update, context):
    echo_mes = ' '.join(context.args)
    context.bot.send_message(chat_id=update.effective_chat.id, text=echo_mes)
...
echo_handler = CommandHandler('echo', echo) #Создаём handler для /echo

dispatcher.add_handler(start_handler)  # Добавляем handler start_handler
dispatcher.add_handler(echo_handler)  # Добавляем handler echo_handler
